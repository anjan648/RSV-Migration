# Requires Az.RecoveryServices and Az.Compute modules
# Install if not present: Install-Module -Name Az -Force
...
foreach ($vm in $vmsToMigrate) {
    ...
    try {
        ...
        if (-not $oldVault) {
            ...
            # -------------------------------
            # Step 2: Stop backup (without deleting recovery points)
            # -------------------------------
            ...
            if ($container) {
                ...
                if ($backupItem) {
                    Disable-AzRecoveryServicesBackupProtection -Item $backupItem -VaultId $oldVault.ID -Force
                    Write-Host "Stopped backup for VM: $($vm.VMName), recovery points retained." -ForegroundColor Yellow
                }
                else {
...
# Export results once after loop
